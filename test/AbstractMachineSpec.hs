...
partialZielcode = Stack [Unify "unify" unify (A "p"), Backtrack "backtrack?" backtrackQ, Push "push" push (A "q"), Call "call" call, Backtrack "backtrack?" backtrackQ, Return "return" returnL]

{-------------------------------------------------- 
    Tests for of codeGen | üb 
 --------------------------------------------------}

testÜbHeadEmptyAkk = TestCase $ 
    assertEqual 
        "übHead should build a Command Stack with correct commands" 
        (Stack [Unify "unify" unify (A "p"), Backtrack "backtrack?" backtrackQ])
        (übHead (A "p") emptyZielcode)

testÜbHeadNonEmptyAkk = TestCase $ 
    assertEqual 
        "übHead should return existing Zielcode appended by head translation rule"
        (partialZielcode <> Stack [Unify "unify" unify (A "p"), Backtrack "backtrack?" backtrackQ])
...
subsequentreg = ((False, Pointer 11, Pointer 8, Pointer 9, Pointer 14), Stack [CodeAddress (Pointer 6), StackAddress Nil, CodeAddress (Pointer 18), CodeAtom (A "p"), CodeAddress (Pointer 12),
                                                                                StackAddress (Pointer 0), CodeAddress (Pointer 5), CodeAtom (A "q"), CodeAddress Nil, StackAddress (Pointer 4), 
                                                                                CodeAddress (Pointer 11), CodeAtom (A "r")])

{--------------------------------------------------- 
    Tests for Helpers
 ---------------------------------------------------}
 
testCGoalPkAndZiel = TestCase $ 
    assertEqual 
        "cGoal should return the Addressvalue of the first command in a goal when LCode consists of pks and goal."
        (Pointer 15) 
        (cGoal code)

testCGoalOnlyZiel = TestCase $ 
    assertEqual 
        "cGoal should return the Addressvalue of the first command in a goal when LCode consists of a single goal."
        (Pointer 0)
        (cGoal code')

testCFirstPkAndZiel = TestCase $ 
    assertEqual 
...
testPushAtomEmptyStack = TestCase $ 
    assertEqual 
...
testPushAtomSubsequent = TestCase $ 
    assertEqual 
...
testReturnLNotNilCase = TestCase $ 
    assertEqual 
    "return on not r unequal nil should set registers correctly"
    ((False, Pointer 11, Pointer 8, Pointer 5, Pointer 11), Stack [CodeAddress (Pointer 6), StackAddress Nil, CodeAddress (Pointer 18), CodeAtom (A "p"), CodeAddress (Pointer 12),
                                                                    StackAddress (Pointer 0), CodeAddress (Pointer 5), CodeAtom (A "q"), CodeAddress Nil, StackAddress (Pointer 4), 
                                                                    CodeAddress (Pointer 11), CodeAtom (A "r")])
    (returnL ((False, Pointer 11, Pointer 8, Pointer 9, Pointer 14), Stack [CodeAddress (Pointer 6), StackAddress Nil, CodeAddress (Pointer 18), CodeAtom (A "p"), CodeAddress (Pointer 12),
                                                                                StackAddress (Pointer 0), CodeAddress (Pointer 5), CodeAtom (A "q"), CodeAddress Nil, StackAddress (Pointer 4), 
                                                                                CodeAddress (Pointer 11), CodeAtom (A "r")]))

testReturnLElseCase = TestCase $ 
    assertEqual 
    ...
    ((False, Pointer 11, Pointer 8, Pointer 5, Pointer 11), Stack [CodeAddress( Pointer 6), StackAddress Nil, CodeAddress (Pointer 18), CodeAtom (A "p"), CodeAddress (Pointer 12),
                                                                                ...
                                                                                CodeAddress (Pointer 11), CodeAtom (A "r")])
    (returnL ((False, Pointer 11, Pointer 8, Pointer 9, Pointer 14), Stack [CodeAddress(Pointer 6), StackAddress Nil, CodeAddress (Pointer 18), CodeAtom (A "p"), CodeAddress (Pointer 12),
                                                                                StackAddress (Pointer 0), CodeAddress (Pointer 5), CodeAtom (A "q"), CodeAddress Nil, StackAddress Nil, 
                                                                                CodeAddress (Pointer 11), CodeAtom (A "r")]))                                                                
